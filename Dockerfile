...
# and `pip install robin` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "robin", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n robin -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "robin", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "robin", "YOUR_CONSOLE_SCRIPT"]
